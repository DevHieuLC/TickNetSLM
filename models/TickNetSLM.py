...
class transitPatterns(torch.nn.Module):
    def __init__(self, in_channels,out_channels, reduction_ratio=16,stride=1):
        super().__init__()
        self.connect = nn.Sequential(
    conv1x1_block(in_channels=in_channels, out_channels=out_channels),
    )
    def forward(self, x):
        ...
        for stage_id, stage_channels in enumerate(channels):
            ...
            for unit_id, unit_channels in enumerate(stage_channels):
                ...
                if in_channels == 512 and unit_channels == 128:                    
                    stage.add_module("Bottleneck{}".format(unit_id + 1), transitPatterns(in_channels=in_channels, out_channels=unit_channels, stride=1))                                        
                else:
